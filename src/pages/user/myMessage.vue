<template>
  <div class="container">
    ...
    <ul v-if="messages.length">
      <li v-for="(message, index) in messages" :key="message.id"> <router-link :to="{ name: 'User', params: { id: message.get('from').id } }">{{ message.get('from').get('username') }}</router-link> 给你发了一条信息
        <p>
